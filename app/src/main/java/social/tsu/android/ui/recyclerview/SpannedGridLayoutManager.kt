...
package social.tsu.android.ui.recyclerview
...
open class SpannedGridLayoutManager(val orientation: Orientation,
    ...
    override fun getDecoratedTop(child: View): Int {
        ...
        var top = childFrames[position]?.top?.plus(decoration)

        if (orientation == Orientation.VERTICAL) {
            top = top?.minus(scroll)
        }
...
open class RectsHelper(val layoutManager: SpannedGridLayoutManager,
     ...
     */
    val itemSize: Int get() {
        val itemS = size / layoutManager.spans
        return if(itemS > 0){
            itemS
        }  else {
            1
