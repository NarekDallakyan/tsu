...
import social.tsu.trimmer.interfaces.OnProgressVideoListener;
import social.tsu.trimmer.interfaces.OnRangeSeekBarListener;
import social.tsu.trimmer.interfaces.OnTrimVideoListener;
import social.tsu.trimmer.utils.BackgroundExecutor;
import social.tsu.trimmer.utils.TrimVideoUtils;
import social.tsu.trimmer.utils.UiThreadExecutor;
import social.tsu.trimmer.view.ProgressBarView;
import social.tsu.trimmer.view.RangeSeekBarView;
import social.tsu.trimmer.view.TimeLineView;

public class TsuVideoTrimmerView extends FrameLayout implements MediaPlayer.OnErrorListener, MediaPlayer.OnPreparedListener, MediaPlayer.OnCompletionListener,
    ...
    public TsuVideoTrimmerView(@NonNull Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void onSave(){
        ...
        if (letUserProceed) {
            if (mStartPosition <= 0 && mEndPosition >= mDuration) {
                mOnTrimVideoListener.onTrimResult(mSrc);
            } else {
                mPlayView.setVisibility(View.VISIBLE);
                mVideoView.pause();
    ...
    private void init(Context context) {
        ...
        if (viewButtonCancel != null) {
            viewButtonCancel.setOnClickListener(view -> mOnTrimVideoListener.onTrimCancel()
            );
        }
