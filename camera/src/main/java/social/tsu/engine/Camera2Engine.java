...
import social.tsu.internal.utils.ImageHelper;
import social.tsu.internal.utils.WorkerHandler;
import social.tsu.picture.Full2PictureRecorder;
import social.tsu.picture.SnapshotGlPictureRecorder;
import social.tsu.preview.GlCameraPreview;
import social.tsu.size.Size;
import social.tsu.size.SizeSelectors;
import social.tsu.video.Full2VideoRecorder;
import social.tsu.video.SnapshotVideoRecorder;
...
public class Camera2Engine extends CameraEngine implements ImageReader.OnImageAvailableListener {
    ...
    private Runnable mAutoFocusResetRunnable = new Runnable() {
        ...
        public void run() {
            ...
            if (maxReagionsAf > 0)
                mRepeatingRequestBuilder.set(CaptureRequest.CONTROL_AF_REGIONS, rectangle);
            if (maxReagionsAe > 0)
                mRepeatingRequestBuilder.set(CaptureRequest.CONTROL_AE_REGIONS, rectangle);
            if (maxReagionsAwb > 0)
                mRepeatingRequestBuilder.set(CaptureRequest.CONTROL_AWB_REGIONS, rectangle);
    ...
    private CameraException createCameraException(int stateCallbackError) {
        ...
        switch (stateCallbackError) {
            ...
            case CameraDevice.StateCallback.ERROR_CAMERA_SERVICE:
                reason = CameraException.REASON_FAILED_TO_CONNECT;
                break; // Fatal error, device might have to be restarted
